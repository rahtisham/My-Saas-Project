...
type PostData = {
    ...
    socialAccount: { id: number; name: string; platform: string };
    media: { id: number; fileName: string; type: string; url: string }[];
};
...
const form = useForm({
    social_account_id: props.post.socialAccount.id.toString(),
    caption: props.post.caption ?? '',
    platform: props.post.platform,
    visibility: props.post.visibility,
    status: props.post.status === 'scheduled' ? 'scheduled' : 'draft',
    scheduled_at: props.post.scheduledAt ? props.post.scheduledAt.slice(0, 16) : '',
    media_ids: props.post.media.map((m) => m.id),
});

const submit = () => {
    form.put(`/${teamSlug.value}/social/posts/${props.post.id}`, {
        preserveScroll: true,
    });
};
...
defineOptions({
    layout: (props: { currentTeam: Team }) => ({
        breadcrumbs: [
            ...
            { title: 'Edit', href: `/${props.currentTeam.slug}/social/posts/${props.post?.id}/edit` },
        ],
    }),
});
</script>
...
<template>
    ...
    <div class="flex flex-col space-y-6">
        <div>
            <h2 class="text-xl font-semibold tracking-tight">Edit Post</h2>
